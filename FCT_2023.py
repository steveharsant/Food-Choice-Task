...
def run():
    ...
    expInfo = {'participant': '', 'order': ['Condition_1_HT', 'Condition_1_TH', 'Condition_2_HT', 'Condition_2_TH'], 'h_list': ['Test', 'Test1', 'Test2', 'foodlist1', 'foodlist2', 'foodlist3', 'foodlist4', 'foodlist5', 'foodlist6'], 't_list': [
        'Test', 'Test1', 'Test2', 'foodlist1', 'foodlist2', 'foodlist3', 'foodlist4', 'foodlist5', 'foodlist6'], 'c_list': ['Test', 'Test1', 'Test2', 'foodlist1', 'foodlist2', 'foodlist3', 'foodlist4', 'foodlist5', 'foodlist6']}
    ...
    for i, row in df.iterrows():
        ...
        for index, food in foodList.iterrows():
            ...
            if key is None:
                ...
                reversed_rating = int(
                    key[0][0]) if condition == 1 else 6 - int(key[0][0])
            food_dict[food_name].append(
                (key[0][0], key[1], reversed_rating, ht_image_onset))
    ...
    for _, food in cList.iterrows():
        ...
        key = showImage(win, food_name, 4.0, constants.C_RATING,
                        zoom=0.5, pos=(0.35, 0.2), ref_image=ref)
        if key is None:
            tooLate = newText(win, "tooLate", "Too late!")
            tooLate.draw()
            win.flip()
    ...
    with open(f"{save_output}.txt", 'w') as f:
        for food, (choice, _, _, available) in choice_dict.items():
            if choice < '3' and available == 1:
                f.write(
                    f"When choosing between [{ref_food}] and [{get_food_name(food)}], you chose [{ref_food}].\n")
                counter += 1
            if choice > '3' and available == 1:
                f.write(
                    f"When choosing between [{ref_food}] and [{get_food_name(food)}], you chose [{get_food_name(food)}].\n")
                counter += 1
            if counter == 5:
                break
    ...
    for food in htcfood:
        ht = food_dict[food] if food in food_dict else [
            (None, None, None, None), (None, None, None, None)]
        c = choice_dict[food] if food in choice_dict else (
            None, None, None, None)
        htc = [*ht]
        htc = htc[0] + htc[1]
        subjinfo = (expInfo['participant'], expInfo['date']
                    [0:9], ref_food, condition)
        extras = (lookup_fat[food]['available'], lookup_fat[food]['fat'],
                  lookup_fat[food]['hilo']) if food in lookup_fat else ()
    ...
    df['rating_reversed'] = df.apply(
        lambda row: 0 if row.condition == 1 else 1, axis=1)
    ...
    cmerged = cList.merge(oList, left_on="food",
                          right_on="food_item", how="outer")

    health = hmerged[["Unnamed: 0", "fat_x", "himage_onset", "h_rt",
                      "health_recorded_response", "available_x", "food"]].copy()
    health.columns = ["t", "fat", "trialonset",
                      "rt", "resp", "available", "food"]
    health["t"] = health["t"] + 1

    taste = tmerged[["Unnamed: 0", "fat_x", "timage_onset", "t_rt",
                     "taste_recorded_response", "available_x", "food"]].copy()
    taste.columns = ["t", "fat", "trialonset",
                     "rt", "resp", "available", "food"]
    taste["t"] = taste["t"] + 1

    choice = cmerged[["Unnamed: 0", "fat_y", "cimage_onset",
                      "c_rt", "choice_rating", "available_y", "food_item"]].copy()
    choice.columns = ["t", "fat", "trialonset",
                      "rt", "resp", "available", "food"]
    choice["t"] = choice["t"] + 1
